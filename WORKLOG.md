...
## 17. Доработки 17 мая 2026 — фильтры, даты, пагинация, билеты

### 17.1 Кабинет начальника (`/manager`)

- Удалён информационный блок **«Power BI и выгрузка»** под таблицей результатов.
- Поле **«Дата смены»**: красный **×** внутри поля → режим **«За всё время»** (сводка, таблица, CSV; сброс попыток за дату в этом режиме отключён).
- Таблица **«Результаты»**: пагинация по **10 сотрудников** на страницу (все попытки выбранных людей); статистика сверху — по всем данным.
- В колонке **«Статус»** у **«Готов»** и **«Не готов»** выводится **дата и время прохождения** (Иркутск), как у «Сброшен».

### 17.2 Список сотрудников (`/manager/workers`)

- Фильтр **«Результат»**: Все / Сдали / Не сдали / Не начинали (параметр `filter` в API).
- **×** у даты смены → просмотр **за всё время** (`shift_date=all`); в строках тестов показывается дата смены.

### 17.3 Вопросы теста (`/manager/test-questions`)

- Кнопка **«Время прохождения билета»** в шапке: лимит **минут на один билет** для теста (`TestType.ticket_time_limit_minutes`, миграция `ensure_test_type_ticket_time_limit`).
- **Удаление билета** (× в шапке билета): подтверждение в модальном окне; удаляются вопросы билета; если есть попытки по билету — ошибка 400.
- Удаление вопроса — через **`confirmDialog`**, не `confirm()`.

### 17.4 Backend

- `GET /api/manager/dashboard`: `shift_date=all`, пагинация `page` / `page_size` (по умолчанию 10 человек).
- `GET /api/manager/workers`: `_parse_workers_shift_date()` для режима «всё время».
- `DELETE /api/manager/test-types/{slug}/tickets/{ticket_id}`.
- `PATCH /api/manager/test-types/{slug}`: опционально `ticket_time_limit_minutes`.

### 17.5 Production

- Изменения выкатывались на **`45-144-220-51.nip.io`** (`scp` + `systemctl restart spvt`, при необходимости `init_db()` для миграции колонки времени).

### 17.6 Переписка

- Обновлён **`cursor_cursor_md.md`**.

---

## 18. Доработки 18 мая 2026 — таймеры, таймаут сдачи, статистика, импорт ГНВП

### 18.1 Несколько правильных ответов

- Модель: `Question.allow_multiple_correct`, `correct_answer` как TEXT (JSON-массив при нескольких верных).
- Утилита `app/question_answer_utils.py` — сериализация и проверка ответа.
- Миграция `ensure_question_multiple_correct()` (снятие блокирующих VIEW перед `ALTER` в PostgreSQL).
- API: создание/редактирование вопросов с `correct_answers`; проверка при `POST /api/test/submit`.
- UI менеджера: галочка «Несколько правильных ответов»; работник — мультивыбор чекбоксами.

### 18.2 Время на вопрос и на билет (по каждому тесту)

- **`TestType.question_time_limit_seconds`** — лимит на один вопрос (только этот тест).
- **`TestType.ticket_time_limit_minutes`** — лимит на весь билет (уже было; доработан показ работнику).
- Сервис `app/services/test_timing.py` — расчёт режима (`question` / `ticket`), лимитов для API и статистики.
- Менеджер (`manager_test_questions.html`): кнопки **«Время на вопрос»** и **«Время прохождения билета»** — PATCH `/api/manager/test-types/{slug}`.
- Работник: таймер в карточке теста; по истечении — **автозавершение с экраном результата**; ответы без выбора считаются пустыми.
- Убрана ошибочная **глобальная** настройка времени через `site_settings` для таймера (ранний вариант отменён).

### 18.3 Таймаут повторной сдачи после успешного прохождения

- **`TestType.retake_after_days`** — через сколько дней после **сдачи** нужно проходить тест снова (на тест).
- Миграция `ensure_test_type_retake_after_days()`.
- Кабинет менеджера: на карточке теста кнопка **«Таймаут теста»** (над «Редактировать билеты»); модальное окно: дни или «Без повторной сдачи по сроку».
- Работник: в кубике теста предупреждение с датой следующей сдачи; `can_start` блокируется до наступления срока.
- Backend: `pass_retake_available_at()` в `test_types.py`; проверка при создании новой попытки в `test.py`.

### 18.4 Статистика начальника

- Колонка **«Время»**: «N из M мин» + подпись «на билет» / «на вопросы» по настройкам теста; для «В процессе» — текущее время с начала попытки.
- Поля в `AttemptSummary`: `elapsed_seconds`, `allotted_seconds`, `time_limit_kind`.

### 18.5 Импорт экзамена ГНВП из текста

- Файл-источник: **`экзамен по ГНВП.txt`** в корне репозитория.
- Скрипт **`scripts/import_gnvp_exam_txt.py`** — тест `gnvp_text_exam`, билеты, вопросы, мультиответы.
- Вспомогательные скрипты: `fix_questions_text_columns.py`, `check_pg_columns.py`.
- На production импорт выполняется на сервере (файл не обязателен в tar-деплое).

### 18.6 Прочее (сессия)

- Фильтры таблицы результатов на `/manager` (ФИО, тест, статус); сортировка попыток **сначала новые**.
- Исправления деплоя: `worker_test.js` (синтаксис), кэш статики (`?v=…`).
- Production: `45.144.220.51` / `45-144-220-51.nip.io`, деплой `tar` + `deploy/post_deploy.sh`.

### 18.7 Новые и изменённые файлы (ориентир)

| Область | Файлы |
|---------|--------|
| Модели / миграции | `models.py`, `schema_migrate.py`, `seed.py` |
| API | `routers/test.py`, `manager.py`, `admin.py`, `schemas.py` |
| Сервисы | `test_types.py`, `test_timing.py`, `question_answer_utils.py`, `site_settings.py` |
| Менеджер | `manager.html`, `manager_tests.js`, `manager_test_questions.js` |
| Работник | `worker_test.js`, `worker_test.html`, `worker_hub.js` |
| Стили | `style.css` |
| Скрипты | `import_gnvp_exam_txt.py`, `fix_questions_text_columns.py`, `check_pg_columns.py` |

### 18.8 Переписка

- Обновлён **`cursor_.md`** (экспорт диалога Cursor, сессия 18.05.2026).
